...
// Trace macros
#define T_E(...) printf(__VA_ARGS__)
#define T_D(...) printf(__VA_ARGS__)
#define T_W(...) printf(__VA_ARGS__)
#define T_I(...) printf(__VA_ARGS__)
...
// Define appl_mcp2210_hidapi_spi. See mcp2210_hidapi_spi.c for implementation details.
extern __attribute__((weak)) mepa_rc mcp2210_hidapi_spi_32bit_read(struct mepa_callout_ctx *ctx,
                                                                mepa_port_no_t port_no,
                                                                const uint8_t mmd,
                                                                const uint16_t addr,
                                                                uint32_t *const value);
extern __attribute__((weak)) mepa_rc mcp2210_hidapi_spi_32bit_write(struct mepa_callout_ctx *ctx,
                                                                mepa_port_no_t port_no,
                                                                const uint8_t mmd,
                                                                const uint16_t addr,
                                                                uint32_t *const value);
...
{
    .spi_read = mcp2210_hidapi_spi_32bit_read,
    .spi_write = mcp2210_hidapi_spi_32bit_write,
...
// *****************************************************************************
 ...
 int main(int argc, const char **argv)
 {
    ...
    printf("VSC8258/Malibu - MCP2210-HIDAPI Win32/Linux Code.\r\n");
    ...
    printf("%s: Connecting to the VSC8258EV/Malibu board via MCP2210-HIDAPI SPI... \r\n", __func__);
    printf("%s: Initializing MCP2210-HIDAPI... \r\n", __func__);
    malibu_mcp2210_board_init(argc, argv);
    ...
    appl_malibu_spi_io_test(&appl_mcp2210_hidapi_spi, appl_callout_ctx);
...
int appl_mepa_init(void)
{
   ...
   // Loop through all ports (PHYs) in the system.
    for (i = 0; i < APPL_PORT_COUNT; ++i)
    {
        ...
        // well).
        appl_malibu_device[i] = mepa_create(&appl_mcp2210_hidapi_spi,
                                            &appl_callout_ctx[i],
                                            &appl_board_conf);
    }
